...
const Dashboard = ({ view }) => {
  ...
  return (
    <div className="min-h-screen text-slate-900 dark:text-white">
      <Navbar isConnected={isConnected} theme={theme} onToggleTheme={handleToggleTheme} />
      ...
      <div className="px-4 pb-12 pt-4 sm:px-6 sm:pt-6">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_280px]">
          ...
          >
            <GlassPanel className="flex flex-col gap-4 p-4">
              <ImpactDetector impact={data?.impact ?? 0} threshold={CONFIG.IMPACT_THRESHOLD} />
              ...
              >
                ...
                <span className="text-xs text-slate-500 dark:text-white/50">Live</span>
              </button>
        ...
        <GlassPanel className="mt-6 px-4 py-4 sm:px-6">
          ...
          <div className="mt-4 grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label} className="rounded-2xl border border-black/10 bg-white/5 px-4 py-3 dark:border-white/10">
        ...
        <footer className="mt-8 flex justify-center text-center text-xs text-slate-500 dark:text-white/50">
          Made by: Hast K. Physics Final Summative Project
        </footer>
      </div>
